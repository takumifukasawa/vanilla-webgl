...
const init = async () => {
  ...
  floorMeshActor.addComponent(
    new ScriptComponent({
      updateFunc: function ({ actor, time, deltaTime }) {
        const t = Matrix4.multiplyMatrices(
          Matrix4.createTranslationMatrix(new Vector3(0, -0.5, 0)),
        );
        actor.worldTransform = t;
      },
    }),
...
const tick = (t) => {
  ...
  // before update
  {
    if (states.isResized) {
      ...
      orthographicCamera.left = -orthographicSize * aspect;
      orthographicCamera.right = orthographicSize * aspect;
      orthographicCamera.bottom = -orthographicSize;
      orthographicCamera.top = orthographicSize;
  ...
  // update
  {
    ...
    orthographicCamera.position.x +=
      (targetX - orthographicCamera.position.x) * dumping;
    orthographicCamera.position.y +=
      (targetY - orthographicCamera.position.y) * dumping;
    ...
    orthographicCamera.cameraMatrix = lookAtCameraMatrix;
  ...
  // render
  {
    ...
    meshActors.forEach((meshActor) => {
      render({
        ...
        viewMatrix: orthographicCamera.cameraMatrix.clone().inverse(),
        projectionMatrix: orthographicCamera.projectionMatrix.clone(),
        normalMatrix: meshActor.worldTransform.clone().inverse().transpose(),
        cameraPosition: orthographicCamera.cameraMatrix.getTranslationVector(),
      });
    });
  }
